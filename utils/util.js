...
function checkClientLogin() {
  ...
  try {
    var csessionId = wx.getStorageSync('sessionId')
    if (csessionId !== null && csessionId !== '') {
      var data = {
        csessionId: csessionId,
      }
      requestLoading(getApp().data.baseUrl + '/ztm/buyer/auth/isLogin', data, "POST", "正在验证身份", function (res) {
        if (res.success) {
          //console.log(res)
          return true;
        } else {
